...
void od::object_detection(const std::vector<cv::Mat>& video_frames, const int n_frame, const std::string bboxes_video_path, const std::vector<cv::Point2f> corners_float, const bool is_distorted, cv::Mat& video_frame, const std::string test_bboxes_video_path, const bool test_flag) {
    ...
    for(size_t i = 0; i < circles.size(); i++) {
        ...
        if (! contours.empty()) {
            ...
            cv::Point2f min_enclosing_center;
            float min_enclosing_radius;
        ...
        double radius_large = radius * 1;
        std::vector<cv::Point2f> circle_points;
        for(size_t j = 0; j < num_points; j++) {
            double angle = 2 * CV_PI * j / num_points;
            cv::Point2f point = center + cv::Point2f(radius_large * cos(angle), radius_large * sin(angle));
            circle_points.push_back(point);
        }
        // Find the minimum enclosing circle
        cv::Point2f min_enclosing_center; float min_enclosing_radius;
        cv::minEnclosingCircle(circle_points, min_enclosing_center, min_enclosing_radius);
        cv::circle(frame, min_enclosing_center, min_enclosing_radius, cv::Scalar(255, 255, 255), 1, cv::LINE_AA);

        // Print min enclosing center and radius
        std::cout << "Center: " << min_enclosing_center << " Radius: " << min_enclosing_radius << std::endl;*/
